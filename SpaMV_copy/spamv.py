...
class SpaMV:
    def __init__(self, adatas: List[AnnData], interpretable: bool, zp_dims: List[int] = None, zs_dim: int = None,
                 ...
                 batch_size: int = None, heads: int = 1,
                 neighborhood_depth: int = 2, neighborhood_embedding: int = 10, random_seed: int = 0,
                 max_epochs_stage1: int = 400, max_epochs_stage2: int = 400, dropout_prob: float = 0, min_kl: float = 1,
                        ...
                        if self.plot and self.epoch > self.pretrain_epoch:
                            if self.interpretable:
                                ...
                                for z in zs:
                                    ...
                                    embedding(self.adatas[0], color=self.adatas[0].obs.columns, basis='spatial',
                                              ncols=5,
                                ...
                                for emb_type in ['all', 'shared', 'Transcriptomics', 'Transcriptomics 2', 'Proteomics',
                                    ...
                                    scanpy.pl.embedding(self.adatas[0], color=name, basis='spatial', ax=ax_list[1][i],
                                                        s=80,
                                                        show=False,
    ...
    def _kl_weight(self):
        kl = self.min_kl + self.epoch / self.max_epochs_stage1 * (self.max_kl - self.min_kl)
        if kl > self.max_kl:
            kl = self.max_kl
        return kl
    ...
    def _kl_weight2(self):
        kl = self.min_kl + (self.epoch2 - self.epoch) / self.max_epochs_stage2 * (self.max_kl - self.min_kl)
        if kl > self.max_kl:
            kl = self.max_kl
        return kl
